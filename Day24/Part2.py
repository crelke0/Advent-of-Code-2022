...
def find_shortest_path(blizards, walls, start_pos, end_pos, width, height, a=False):
  g_costs = {start_pos: 0}
  ...
  while len(_open):
    ...
    for n in ((x - 1, y, t + 1), (x + 1, y, t + 1), (x, y - 1, t + 1), (x, y + 1, t + 1), (x, y, t + 1)):
      nxy = n[:-1]
      if n[0] < 0 or n[0] >= width or nxy in walls or nxy in timed_blizards[n[2]]: continue
      if nxy == end_pos[:-1]:
        return current_g_cost + 1, timed_blizards[-1]
      if n not in g_costs:
...
a, blizards = find_shortest_path(blizards, walls, start_pos, end_pos, width, height)
b, blizards = find_shortest_path(blizards, walls, end_pos, start_pos, width, height)
c, _ = find_shortest_path(blizards, walls, start_pos, end_pos, width, height)
print(a + b + c)
